...
func (dvp DVP) ArdsCallback() string {
	...
	SetSessionInfo(reqOData.CampaignId, ardsCallbackInfo.SessionID, "AgentSipName", ardsCallbackInfo.ResourceInfo.ContactName)
	...
	if RedisCheckKeyExist(sessionInfoKey) {
		if rdata.Reply.Message == "ACCEPTED" {
			...
			if previewReAssignOnFail == "false" {
				...
				sessionInfo["AgentReasonCode"] = rdata.Reply.Message
				redGreen.Println(fmt.Sprintf("=========== PREVIEW REJECTED - SESSION INFO : %v ==========", sessionInfo))
				go ManageIntegrationData(sessionInfo, "AGENT")
			} else {
