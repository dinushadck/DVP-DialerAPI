	...
	if eventInfo != defEventAgent {
		...
		if eventInfo.TenantId != "" && eventInfo.CompanyId != "" {
			...
			case "CHANNEL_CREATE":
				...
				color.Cyan(fmt.Sprintf("EventName: %s, SessionId: %s, EventCat: %s", eventInfo.EventName, eventInfo.SessionId, eventInfo.EventCategory))
				redGreen.Println(fmt.Sprintf("EventName: %s, SessionId: %s, EventCat: %s", eventInfo.EventName, eventInfo.SessionId, eventInfo.EventCategory))
				break
			case "CHANNEL_ANSWER":
				SetSessionInfo(eventInfo.CampaignId, eventInfo.SessionId, "DialerStatus", "channel_answered")
				SetSessionInfo(eventInfo.CampaignId, eventInfo.SessionId, "ChannelAnswertime", time.Now().Format(layout4))
