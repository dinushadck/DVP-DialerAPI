...
func DialNumberFIFO(company, tenant int, resourceServer ResourceServerInfo, campaignId, scheduleId, campaignName, uuid, fromNumber, trunkCode, phoneNumber, xGateway, extention string) {
	...
	InitiateSessionInfo(company, tenant, 240, "Campaign", "Dialer", "FIFODial", "1", campaignId, scheduleId, campaignName, uuid, phoneNumber, "start", "dial_start", time.Now().Format(layout4), resourceServer.ResourceServerId, nil)
