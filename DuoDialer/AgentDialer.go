...
func AddAgentDialRequest(company, tenant int, resourceServer ResourceServerInfo, campaignId, scheduleId, campaignName, dialoutMec, uuid, fromNumber, trunkCode, phoneNumber, xGateway, numExtraData, tryCount, extention string) {
	...
	InitiateSessionInfo(company, tenant, 240, "Campaign", "Dialer", "AgentDial", tryCount, campaignId, scheduleId, campaignName, uuid, phoneNumber, "ards added", "dial_start", time.Now().UTC().Format(layout4), resourceServer.ResourceServerId)
	...
	if RedisCheckKeyExist(sessionInfoKey) {
		...
		if fromNumber != "" && trunkCode != "" && phoneNumber != "" && extention != "" {
			...
			*/
			...
			if contactType == "PRIVATE" {
				dial = true
				param = fmt.Sprintf(" {sip_h_DVP-DESTINATION-TYPE=PRIVATE_USER,DVP_CUSTOM_PUBID=%s,CustomCompanyStr=%s,CampaignId=%s,ards_resource_id=%s,tenantid=%s,companyid=%s,ards_client_uuid=%s,origination_uuid=%s,ards_servertype=%s,ards_requesttype=%s,DVP_ACTION_CAT=DIALER,return_ring_ready=false,ignore_early_media=true,origination_caller_id_number=%s}", subChannelName, customCompanyStr, campaignId, resourceId, tenant, company, sessionId, sessionId, ardsServerType, ardsReqType, phoneNumber)
			...
			if xGateway != "" {
				...
				data = fmt.Sprintf(" &bridge({sip_h_DVP-DESTINATION-TYPE=GATEWAY,CampaignId=%s,CustomCompanyStr=%s,OperationType=Dialer,origination_caller_id_number=%s,originate_timeout=30,sip_h_X-Gateway=%s}sofia/gateway/%s/%s)", campaignId, customCompanyStr, fromNumber, xGateway, trunkCode, phoneNumber)
			} else {
				data = fmt.Sprintf(" &bridge({sip_h_DVP-DESTINATION-TYPE=GATEWAY,CampaignId=%s,CustomCompanyStr=%s,OperationType=Dialer,origination_caller_id_number=%s,originate_timeout=30}sofia/gateway/%s/%s)", campaignId, customCompanyStr, fromNumber, trunkCode, phoneNumber)
			}
