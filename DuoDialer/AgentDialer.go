...
func AddAgentDialRequest(company, tenant int, resourceServer ResourceServerInfo, campaignId, scheduleId, campaignName, dialoutMec, uuid, fromNumber, trunkCode, phoneNumber, xGateway, numExtraData, tryCount, extention string, integrationData *IntegrationConfig, contacts *[]Contact) {
	...
	InitiateSessionInfo(company, tenant, 240, "Campaign", "Dialer", "AgentDial", tryCount, campaignId, scheduleId, campaignName, uuid, phoneNumber, "ards added", "dial_start", time.Now().UTC().Format(layout4), resourceServer.ResourceServerId, integrationData, contacts, "")
	...
	if RedisCheckKeyExist(sessionInfoKey) {
		...
		if fromNumber != "" && trunkCode != "" && phoneNumber != "" && extention != "" {
			...
			*/
			...
			dial = true
			param = fmt.Sprintf("<DVP_CALL_DIRECTION=outbound,tenantid=%s,companyid=%s>{sip_h_DVP-DESTINATION-TYPE=GATEWAY,ards_skill_display=%s,DVP_CUSTOM_PUBID=%s,nolocal:DIALER_AGENT_EVENT=%s,CustomCompanyStr=%s,CampaignId=%s,CampaignName='%s',ards_resource_id=%s,ards_client_uuid=%s,origination_uuid=%s,ards_servertype=%s,ards_requesttype=%s,DVP_ACTION_CAT=DIALER,DVP_OPERATION_CAT=AGENT,return_ring_ready=false,ignore_early_media=true,origination_caller_id_number=%s,nolocal:CALL_LEG_TYPE=AGENT}", tenant, company, ardsQueueName, subChannelName, subChannelNameAgent, customCompanyStr, campaignId, campaignName, resourceId, sessionId, sessionId, ardsServerType, ardsReqType, phoneNumber)
			furl = fmt.Sprintf("sofia/gateway/%s/%s", trunkCode, phoneNumber)
